...
exports.userInsert = (req, res) =>{
	...
	User.findOne({ nickName: user.nickName }, (err, userMongo) => {
			if (err) return res.status(500).send(err.message)
            if (userMongo) return res.status(500).send('this user is already used')   
            if (!params.password) return res.status(500).send('Mandatory password')   
            if (!user.nickName) return res.status(500).send('Mandatory nickName')
